    ...
    def default(self, o):
        if isinstance(o, datetime.datetime):
            if o.microsecond == 0:
                ...
                # things smaller.
                return "Date({yr},{mo},{dy},{hr},{mn},{sc})".format(yr=o.year, mo=o.month - 1, dy=o.day, hr=o.hour,
                                                                    mn=o.minute, sc=o.second)
            else:
                return "Date({yr},{mo},{dy},{hr},{mn},{sc},{ms})".format(yr=o.year, mo=o.month - 1, dy=o.day, hr=o.hour,
                                                                         mn=o.minute, sc=o.second,
                                                                         ms=o.microsecond / 1000)
        elif isinstance(o, datetime.date):
            return "Date({yr},{mo},{dy})".format(yr=o.year, mo=o.month - 1, dy=o.day)
...
class DataTable(object):
    ...
    def __init__(self, table_description, data=None, custom_properties=None):
        ...
        """
        self.__columns = self.table_description_parser(table_description)
        self.__data = []
    ...
    @staticmethod
    def escape_for_js_code(encoder, value):
        if value is None:
            return "null"
        elif isinstance(value, datetime.datetime):
            if value.microsecond == 0:
                # If it's not ms-resolution, leave that out to save space.
                return "new Date({yr},{mo},{dy},{hr},{mn},{sc})".format(yr=value.year,
                                                                        mo=value.month - 1,
                                                                        dy=value.day,
                                                                        hr=value.hour,
                                                                        mn=value.minute,
                ...
                return "new Date({yr},{mo},{dy},{hr},{mn},{sc}, {ms})".format(yr=value.year,
                                                                              mo=value.month - 1,
                                                                              dy=value.day,
                                                                              hr=value.hour,
                                                                              mn=value.minute,
            ...
            return "new Date({yr},{mo},{dy})".format(yr=value.year, mo=value.month - 1, dy=value.day)
        else:
    ...
    def table_description_parser(table_description, depth=0):
        ...
        parsed_col = DataTable.column_type_parser(list(table_description.keys())[0])
        parsed_col["depth"] = depth
        parsed_col["container"] = "dict"
        return ([parsed_col] +
                DataTable.table_description_parser(list(table_description.values())[0],
                                                   depth=depth + 1))
    ...
    def _inner_append_data(self, prev_col_values, data, col_index):
        ...
        if not data.keys():
            ...
            for key in sorted(data):
                ...
                self._inner_append_data((col_values, prev_col_values[1]),
    ...
    def _prepared_data(self, order_by=()):
        ...
        proper_sort_keys = []
        if isinstance(order_by, str) or (isinstance(order_by, tuple) and len(order_by) == 2 and
        ...
        return sorted(self.__data, key=functools.cmp_to_key(sort_cmp_func))

    def to_js_code(self, name, columns_order=None, order_by=()):
        ...
        for i, col in enumerate(columns_order):
            jscode += "{nm}.addColumn({tp}, {lb}, {id});\n".format(
                nm=name,
                tp=encoder.encode(col_dict[col]["type"]),
                lb=encoder.encode(col_dict[col]["label"]),
                id=encoder.encode(col_dict[col]["id"]))
            if col_dict[col]["custom_properties"]:
                jscode += "{nm}.setColumnProperties({i}, {cs});\n".format(
                    nm=name, i=i, cs=encoder.encode(col_dict[col]["custom_properties"]))
        jscode += "{nm}.addRows({ln});\n".format(nm=name, ln=len(self.__data))

        # We now go over the data and add each row
        for (i, (row, cp)) in enumerate(self._prepared_data(order_by)):
            ...
            for (j, col) in enumerate(columns_order):
                ...
                if isinstance(value, tuple):
                    cell_cp = ""
                    if len(value) == 3:
                        cell_cp = ", {cl}".format(cl=encoder.encode(row[col][2]))
                    # We have a formatted value or custom property as well
                    jscode += ("{nm}.setCell({i}, {j}, {ev}, {ej}{cl});\n".format(nm=name, i=i, j=j,
                                ev=self.escape_for_js_code(encoder, value[0]),
                                ej=self.escape_for_js_code(encoder, value[1]), cl=cell_cp))
                else:
                    jscode += "{nm}.setCell({i}, {j}, {ej});\n".format(
                        nm=name, i=i, j=j, ej=self.escape_for_js_code(encoder, value))
            if cp:
                jscode += "{nm}.setRowProperties({i}, {cp});\n".format(
                    nm=name, i=i, cp=encoder.encode(cp))
        return jscode

    def to_html(self, columns_order=None, order_by=()):
        ...
        for col in columns_order:
            columns_list.append(header_cell_template %
                                cgi.html.escape(col_dict[col]["label"]))
        columns_html = columns_template % "".join(columns_list)

        rows_list = []
        # We now go over the data and add each row
        for row, unused_cp in self._prepared_data(order_by):
            cells_list = []
            # We add all the elements of this row by their order
            for col in columns_order:
                ...
                if isinstance(value, tuple):
                    ...
                    cells_list.append(cell_template % cgi.html.escape(self.to_string(value[1])))
                else:
                    cells_list.append(cell_template % cgi.html.escape(self.to_string(value)))
    ...
    def to_csv(self, columns_order=None, order_by=(), separator=","):
            ...
            for col in columns_order:
                ...
                if isinstance(value, tuple):
                    ...
                    if col_dict[col]["type"] in ["date", "datetime", "timeofday"]:
                        cells_list.append(self.to_string(value[1]).encode("utf-8"))
                    else:
                        cells_list.append(self.to_string(value[0]).encode("utf-8"))
                else:
                    cells_list.append(self.to_string(value).encode("utf-8"))
            writer.writerow(cells_list)
        return csv_buffer.getvalue()

    def to_tsv_excel(self, columns_order=None, order_by=()):
        ...
        """
        return (self.to_csv(columns_order, order_by, separator="\t")
    ...
    def to_json_response(self, columns_order=None, order_by=(), req_id=0,
        ...
        response_obj = {
            "version": "0.6",
            "reqId": str(req_id),
            "table": self._to_json_obj(columns_order, order_by),
            "status": "ok"
        }
        encoder = DataTableJSONEncoder()
        return "{rh}({ro});".format(rh=response_handler, ro=encoder.encode(response_obj))

    def to_response(self, columns_order=None, order_by=(), tqx=""):
        ...
        if tqx_dict.get("out", "json") == "json":
            response_handler = tqx_dict.get("responseHandler",
                                            "google.visualization.Query.setResponse")
            return self.to_json_response(columns_order, order_by,
                                         req_id=tqx_dict.get("reqId", 0),
                                         response_handler=response_handler)
        elif tqx_dict["out"] == "html":
            return self.to_html(columns_order, order_by)
        elif tqx_dict["out"] == "csv":
            return self.to_csv(columns_order, order_by)
        elif tqx_dict["out"] == "tsv-excel":
            return self.to_tsv_excel(columns_order, order_by)
        else:
            raise DataTableException(
                "'out' parameter: '{td}' is not supported".format(td=tqx_dict["out"]))
